...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Superuser credentials check
    if ($email === 'root' && $password === 'T9x!rV@5mL#8wQz&Kd3') {
        $_SESSION['user_id'] = '5'; // Use a placeholder session for superuser
        $_SESSION['role'] = 'admin'; // Mark as superuser
        $_SESSION['name'] = 'root';
        $_SESSION['email'] = 'root@gmail.com';

        // Redirect to all-link.php for superuser
        header("Location: all-link.php");
        exit();
    }

    // Normal user/admin login process
    ...
    <div class="container mx-auto p-6 max-w-md bg-white shadow-md rounded-lg mt-10">
        ...
        <form method="POST" action="">
            <div class="form-group mb-4">
                <input type="text" name="email" placeholder="Email" required class="w-full p-3 border border-gray-300 rounded-md">
            </div>
            <div class="form-group mb-4">
                <input type="password" name="password" placeholder="Password" required class="w-full p-3 border border-gray-300 rounded-md">
            </div>
            <div class="form-group mb-4">
                <select name="role"  class="w-full p-3 border border-gray-300 rounded-md">
